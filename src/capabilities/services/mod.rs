...
pub(crate) fn validate_unit(name: &str) -> Result<Cow<'_, str>> {
    ...
    if name.len() > MAX_LEN {
        return Err(FezError::Usage(format!(
            "unit name too long ({len} > {MAX_LEN})",
            len = name.len()
        )));
    }
...
fn validate_log_priority(raw: &str) -> Result<()> {
    if raw.is_empty() || raw.starts_with('-') {
        return Err(FezError::Usage(format!(
            "invalid --priority value: {raw}"
        )));
    }
    if LOG_PRIORITIES.iter().any(|level| level.eq_ignore_ascii_case(raw)) {
        return Ok(());
    }
...
fn validate_and_mutate(cli: &Cli, action: &ServicesAction, unit: &str) -> Result<View> {
    ...
    match action {
        ...
        _ => unreachable!("validate_and_mutate only called for mutations"),
    }
}

#[cfg(test)]
mod tests {
    use super::{mangle_unit, validate_unit, validate_log_priority, validate_log_since};
    ...
    fn validate_rejects_path_separators() {
        assert!(matches!(validate_unit("../../etc/shadow"), Err(FezError::Usage(_))));
        assert!(matches!(validate_unit("/usr/lib/systemd/system/evil.service"), Err(FezError::Usage(_))));
    }

    #[test]
    fn validate_rejects_control_chars() {
        assert!(matches!(validate_unit("foo\x00bar"), Err(FezError::Usage(_))));
        assert!(matches!(validate_unit("foo\nbar"), Err(FezError::Usage(_))));
        assert!(matches!(validate_unit("foo\x1bbar"), Err(FezError::Usage(_))));
    }
    ...
    fn validate_accepts_normal_units() {
        assert_eq!(validate_unit("sshd.service").unwrap().as_ref(), "sshd.service");
        assert_eq!(validate_unit("chronyd").unwrap().as_ref(), "chronyd.service");
    }

    #[test]
    fn validate_allows_known_chars_in_unit_names() {
        // systemd allows alphanumerics, `:`, `@`, `.`, `_`, `\\`, `-`
        assert_eq!(validate_unit("getty@tty1.service").unwrap().as_ref(), "getty@tty1.service");
        assert_eq!(
    ...
    fn validate_log_priority_rejects_bad_values() {
        ...
        assert!(matches!(validate_log_priority("8"), Err(FezError::Usage(_))));
