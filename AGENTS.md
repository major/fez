...
- `packages` subcommands: mutations `install`/`remove`/`upgrade`/`distro-sync`/`downgrade`/`reinstall` (all go through the resolve-first/guardrail/`do_transaction` pipeline; the `Mutation::verb` user-facing verb and `Mutation::method` D-Bus method can diverge, e.g. `distro-sync` -> `distro_sync`) and reads `list`/`info`/`search`/`check-update`/`repolist`/`advisories`/`history`. The D-Bus surface fez calls spans `rpm.Rpm` (`list`, `install`, `remove`, `upgrade`, `distro_sync`, `downgrade`, `reinstall`), `rpm.Repo` (`list`), `Goal` (`resolve`, `do_transaction`), `Advisory` (`list`, behind `advisories`), and `History` (`recent_changes`, behind `history`). The `Advisory`/`History` interfaces are `org.rpm.dnf.v0.Advisory`/`org.rpm.dnf.v0.History` (no `.rpm.` segment, unlike `rpm.Rpm`/`rpm.Repo`). The fake bridge disambiguates the shared `list` method by interface suffix (`.ends_with(".Advisory")`, `.ends_with(".rpm.Repo")`).
