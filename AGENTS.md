...
- The fake bridge completes the **superuser handshake**: on `init` it emits `superuser-init-done`. `FEZ_FAKE_SUPERUSER` selects the escalation behavior: `ok`/unset (passwordless success), `challenge` (sends an `authorize` password prompt the client must refuse), `denied` (init succeeds but every `superuser: "require"` channel closes with `access-denied`, mirroring a host whose standalone bridge has no superuser bridge config). The `challenge` and `denied` paths both drive fez to exit 11 (`access-denied`). Real standalone `cockpit-bridge` (no `cockpit-system`) has **zero** superuser bridge configs, so `superuser: {id: "sudo"}` is an unknown type and privileged channels are denied; escalation requires `cockpit-system` (it ships the sudo/pkexec bridge definitions in its shell manifest) plus passwordless sudo. fez does not supply sudo passwords.
